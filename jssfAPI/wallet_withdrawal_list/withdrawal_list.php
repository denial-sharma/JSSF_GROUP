...
if (isset($_REQUEST['decoded_token'])) {
    ...
    try {
        $getWalletAmt = $conn->prepare("SELECT * FROM withdrawal WHERE `userid` = '$userid' ");
        $getWalletAmt->execute();

        // Check if the statement execution was successful
        if ($getWalletAmt) {
            $getWithdrawalList = [];
            $getWithdrawalList = $getWalletAmt->fetchall();
            if ($getWithdrawalList) {

                echo json_encode(['status' => 200, 'withdrawalList' => $getWithdrawalList]);
            } else {

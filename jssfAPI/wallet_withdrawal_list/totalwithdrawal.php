...
if (isset($_REQUEST['decoded_token'])) {
    ...
    try {
        $getTotalWithdrawalAmount = $conn->prepare("SELECT sum(amount) as withdrdrawalamount FROM withdrawal WHERE `userid` = '$userid'  ");
        $getTotalWithdrawalAmount->execute();

        // Check if the statement execution was successful
        if ($getTotalWithdrawalAmount) {
            $row = $getTotalWithdrawalAmount->fetch();
            $getTotalAmount = $row['withdrdrawalamount'];

            if ($getTotalAmount) {

                echo json_encode(['status' => true, 'message' => $getTotalAmount]);
            } else {
                http_response_code(404);  // Set response code to 404 Not Found
                die(json_encode(['status' => false, 'message' => 0]));
            }
        } else {

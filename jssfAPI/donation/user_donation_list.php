...
if (isset($_REQUEST['decoded_token'])) {
    ...
    try {
        $getdonation = $conn->prepare("SELECT * FROM `transaction` where userid =  '$userid'  And `type_trans` = 'donation' ");
        $getdonation->execute();

        // Check if the statement execution was successful
        if ($getdonation) {
            $getDonationList = $getdonation->fetchall(PDO::FETCH_ASSOC);
            if ($getDonationList) 

                echo json_encode(['status' => true, 'donationList' => $getDonationList]);
            } else {
